...
class AnalyticsWalletAdmin(admin.ModelAdmin):
    ...
    actions = ['calculate_total_balance', 'export_wallet_data']
    ...
    def calculate_total_balance(self, request, queryset):
        total = sum(float(w.balance) for w in queryset)
        self.message_user(request, f"Total balance: {total:,.2f}")
    calculate_total_balance.short_description = 'Calculate total balance'

    def export_wallet_data(self, request, queryset):
        self.message_user(request, f"Exporting data for {queryset.count()} wallets")
